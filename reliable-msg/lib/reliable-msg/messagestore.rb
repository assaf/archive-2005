        ...
        module MessageStore #:nodoc:
            ...
            begin
                ...
                class MySQL < Base #:nodoc:
                    ...
                    THREAD_CURRENT_MYSQL = :rubysteps_mysql #:nodoc:
                ...
                protected
                    ...
                    def update inserts, deletes, dlqs
                        ...
                        begin
                            inserts.each do |insert|
                                mysql.query "INSERT INTO `#{@prefix}messages` (id,queue,headers,object) VALUES('#{connection.quote  insert[:id]}','#{connection.quote insert[:queue]}',BINARY '#{connection.quote Marshal::dump(insert[:headers])}',BINARY '#{connection.quote insert[:message]}')"
                            end
                            ids = deletes.collect {|delete| "'#{delete[:id]}'" }
                            if !ids.empty?
                                mysql.query "DELETE FROM messages WHERE id IN (#{ids.join ','})"
                            end
                            dlqs.each do |dlq|
                                mysql.query "UPDATE `#{@prefix}messages` SET queue='#{Queue::DLQ}' WHERE id='#{connection.quote dlq[:id]}'"
                            end
                            mysql.query "COMMIT"
                        rescue Exception=>error
                    ...
                    def load_index
                        connection.query "SELECT id,queue,headers FROM `#{@prefix}messages`" do |result|
                    ...
                    def load id, queue
                        message = nil
                        connection.query "SELECT object FROM `#{@prefix}messages` WHERE id='#{id}'" do |result|
