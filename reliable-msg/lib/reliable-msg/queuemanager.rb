...
require 'rubysteps/queues/messagestore'
        ...
        class QueueManager
            ...
            def start
                @mutex.synchronize do
                    ...
                    store_cls = RubySteps::Queues::MessageStore
                    ...
                    if @logger
                        @logger.info "Using message store #{store_cls}"
                    else
                        warn "#{Queue::PACKAGE}: Using message store #{store_cls}"
                    end
                    ...
                    if @logger
                        ...
                        warn "#{Queue::PACKAGE}: Accepting requests at 'druby://localhost:#{port}'"
                    end
                                    ...
                                    if tx[:timeout] <= time
                                        begin
                                            if @logger
                                                ...
                                                warn "#{Queue::PACKAGE}: Timeout: aborting transaction #{tid}"
                                            end
                                            abort tid
                                        rescue
                                        end
    ...
    puts 'BLAFKJADFKADJFDAKJ'
            ...
            def stop
                @mutex.synchronize do
                    ...
                    if @logger
                        ...
                        warn "#{Queue::PACKAGE}: Stopped queue manager at '#{drb_uri}'"
                    end
                end
            end
            ...
            def abort tid
                ...
                if @logger
                    ...
                    warn "#{Queue::PACKAGE}: Transaction #{tid} aborted"
                end
            end
