...
class HTMLParser < SGMLParser
    ...
    def handle_data(data)
        @current << REXML::Text.new(data) if @current
    end
    ...
    def unknown_starttag(tag, attrs)
        element = REXML::Element.new(tag.downcase)
        attrs.each do |key, value| element.add_attribute(key.downcase, value) end
        if @current
            @current << element
        else
            @document << element
    ...
    def unknown_entityref(ref)
        @current << REXML::Text.new("&amp;#{ref}&lt;") if @current
    end
